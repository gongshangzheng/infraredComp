...
>
> 已有：`compressai-usage`（CompressAI：image + ssf2020 video，codec 已建 `ssf2020.py` 可训练）、`dcvc-rt-usage`（microsoft/DCVC-RT，codec 已建 `dcvc_rt.py` 推理专用无训练）。集成新库 X 时建 `x-usage`。
