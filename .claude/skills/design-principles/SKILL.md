...
**原则**：当页面列出多项结果（论文、图片、视频、任务、成员等）时，**必须分页**，不要一次渲染全部。**统一用页码分页（`<n-pagination>`），不用「加载更多」按钮。**

**为什么**：
- 数据量不确定（可能几十条也可能几百条），全量渲染会导致页面卡顿。
- 用户浏览习惯是一页一页翻，不是一次性看全部。
- 分页让前端组件保持响应性，避免大数据量下 DOM 节点过多。
- 页码分页可定位页数、可前后跳，与搜索引擎浏览习惯一致；「加载更多」无法跳页、累计状态难推断、刷新后丢失位置。
...
| 卡片/图片网格 | 页码器 | `<n-pagination>`（远程 offset/limit + 服务端筛选；小数据量可客户端 slice） | 12–24 |
| 详情内嵌列表 | 页码器（客户端 slice） | `<n-pagination>` over 已取回的数组 | 12 |
| 输出文件/日志 | 远程分页 + 页码器 | `<n-pagination>` | 100 |

**镜像参考**：`web/src/views/papers/PaperList.vue` 的 `<n-pagination v-model:page="currentPage" :page-count="Math.ceil(filtered.length/pageSize)" :page-size="pageSize" />` 是卡片网格客户端 slice 分页的范例。
