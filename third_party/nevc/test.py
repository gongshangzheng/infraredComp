...
# Line 41 is modified by ByteDance
from src.transforms.functional import ycbcr444_to_420, ycbcr420_to_444, rgb_to_ycbcr444_bt601,ycbcr_to_rgb_bt601
...
def parse_args():
    ...
    # Line 64 is modified by ByteDance
    parser.add_argument("--output_frame_RD", type=str2bool, nargs='?', const=True, default=False)
...
def run_test(p_frame_net, i_frame_net, args):
    ...
    # Line 102 is modified by ByteDance
    reset_interval = args['reset_interval']

    if args['src_type'] == 'png':
        src_reader_rgb = PNGReader(args['src_path'], args['src_width'], args['src_height'])
    ...
    if save_decoded_frame:
        if args['src_type'] == 'png':
            recon_writer_rgb = PNGWriter(args['recon_path'], args['src_width'], args['src_height'])
        elif args['src_type'] == 'yuv420':
            recon_writer = YUVWriter(args['recon_path'], args['src_width'], args['src_height'])

    frame_types = []
    psnrs_rgb = []
    # Line 119-120 is modified by ByteDance
    psnrs_yuv444 = []
    psnrs_yuv420 = []
    psnrs_y = []
    psnrs_u = []
    psnrs_v = []
    msssims_rgb = []
    # Line 126-127 is modified by ByteDance
    msssims_yuv444 = []
    msssims_yuv420 = []
    msssims_y = []
    msssims_u = []
    ...
    with torch.no_grad():
        # Line 141-144 is modified by ByteDance
        if args['dist_in_yuv420']:
            next_y, next_uv = src_reader.read_one_frame(dst_format="420")
        else:
            next_rgb = src_reader_rgb.read_one_frame(dst_format="rgb")
        for frame_idx in range(frame_num):
            frame_start_time = time.time()
            if args['dist_in_yuv420']:
                # Line 149-159 is modified by ByteDance
                y = next_y
                uv = next_uv
                ...
                if frame_idx != frame_num - 1:
                    next_y, next_uv = src_reader.read_one_frame(dst_format="420")
                    next_yuv = ycbcr420_to_444(next_y, next_uv, order=0)
                    x_next = np_image_to_tensor(next_yuv)
                yuv = ycbcr420_to_444(y, uv, order=0)
                rgb=ycbcr_to_rgb_bt601(yuv)
                ...
                # Line 166-175 is modified by ByteDance
                rgb = next_rgb
                
                x_next = None
                if frame_idx != frame_num - 1:
                    next_rgb = src_reader_rgb.read_one_frame(dst_format="rgb")
                    x_next = np_image_to_tensor(next_rgb)
                y, uv = rgb_to_ycbcr444_bt601(rgb)
                y = y[0, :, :]
                u = uv[0, :, :]
                v = uv[1, :, :]
            ...
            x = x.to(device)
            # Line 180-181 is modified by ByteDance
            ...
            # Line 198-204 is modified by ByteDance
            x_next_padded = None
            if x_next is not None:
                x_next_padded = torch.nn.functional.pad(
                    x_next,
                    (padding_l, padding_r, padding_t, padding_b),
                    mode="replicate",
                )
            ...
            if frame_idx % gop_size == 0:
                ...
                dpb = {
                    ...
                    "ref_y": None,
                    "ref_mv_y": None,
                    # Line 219-220 are modified by ByteDance
                    "key_feature": None,
                    "key_mvs": [],
                }
                ...
                # Line 227-230 are modified by ByteDance
                if reset_interval > 0 and frame_idx % reset_interval == 1:
                    dpb["ref_feature"] = None
                    dpb["key_feature"] = None
                    dpb["key_mvs"] = []

                # Line 234 is modified by ByteDance
                result = p_frame_net.encode_decode(x_padded, dpb, args['q_in_ckpt'],
                                                                            args['i_frame_q_index'], x_next_padded, bin_path,
                                                                   pic_height=pic_height, pic_width=pic_width,
            ...
            if args['dist_in_yuv420']:
                # Line 249-250 is modified by ByteDance
                rgb_rec=ycbcr_to_rgb_bt601(x_hat)  
                psnr_rgb = PSNR(rgb_rec, rgb)
                ...
                psnr_yuv420 = (6 * psnr_y + psnr_u + psnr_v) / 8
                if args['calc_ssim']:
                    # Line 263 is modified by ByteDance
                    ssim_rgb = ms_ssim(rgb_rec, rgb, data_range=1).item()
                    ssim_y = calc_msssim(y, y_rec, data_range=1)
                    ssim_u = calc_msssim(u, u_rec, data_range=1)
                    ssim_v = calc_msssim(v, v_rec, data_range=1)
                else:
                    # Line 269 is modified by ByteDance
                    ssim_rgb=0
                    ssim_y, ssim_u, ssim_v = 0., 0., 0.
                ssim = (6 * ssim_y + ssim_u + ssim_v) / 8
                psnrs_rgb.append(psnr_rgb)
                # Line 274 is modified by ByteDance
                psnrs_yuv420.append(psnr_yuv420)
                psnrs_y.append(psnr_y)
                psnrs_u.append(psnr_u)
                psnrs_v.append(psnr_v)
                # Line 279 is modified by ByteDance
                msssims_yuv420.append(ssim)
                msssims_y.append(ssim_y)
                msssims_u.append(ssim_u)
                msssims_v.append(ssim_v)
            else:

                psnr_rgb = PSNR(x_hat, x)
                # Line 287-295 is modified by ByteDance
                yuv_rec = x_hat.squeeze(0).cpu().numpy()
                y_rec, uv_rec = rgb_to_ycbcr444_bt601(yuv_rec)
                y_rec = y_rec[0, :, :]
                u_rec = uv_rec[0, :, :]
                v_rec = uv_rec[1, :, :]
                psnr_y = calc_psnr(y, y_rec, data_range=1)
                psnr_u = calc_psnr(u, u_rec, data_range=1)
                psnr_v = calc_psnr(v, v_rec, data_range=1)
                psnr_yuv444 = (6 * psnr_y + psnr_u + psnr_v) / 8

                if args['calc_ssim']:
                    # Line 299-302 is modified by ByteDance
                    ssim_rgb = ms_ssim(x_hat, x, data_range=1).item()
                    ssim_y = calc_msssim(y, y_rec, data_range=1)
                    ssim_u = calc_msssim(u, u_rec, data_range=1)
                    ssim_v = calc_msssim(v, v_rec, data_range=1)
                else:
                    # Line 305-317 is modified by ByteDance
                    ssim_rgb=0
                    ssim_y, ssim_u, ssim_v = 0., 0., 0.
                ssim = (6 * ssim_y + ssim_u + ssim_v) / 8
                psnrs_rgb.append(psnr_rgb)
                psnrs_yuv444.append(psnr_yuv444)
                psnrs_y.append(psnr_y)
                psnrs_u.append(psnr_u)
                psnrs_v.append(psnr_v)
                msssims_rgb.append(ssim_rgb)
                msssims_yuv444.append(ssim)
                msssims_y.append(ssim_y)
                msssims_u.append(ssim_u)
                msssims_v.append(ssim_v)
            ...
            if verbose >= 2:
                ...
                print(f"frame {frame_idx}, {frame_end_time - frame_start_time:.3f} seconds,",
                      f"bits: {bits[-1]/frame_pixel_num:.3f}, PSNR_RGB: {psnrs_rgb[-1]:.4f},PSNR_YUV444:{psnrs_yuv444[-1]:.4f}, MS-SSIM_RGB: {msssims_rgb[-1]:.4f},MS-SSIM_YUV444:{msssims_yuv444[-1]:4f} ")
            ...
            if save_decoded_frame:
                if args['dist_in_yuv420']:
                    ...
                    recon_writer_rgb.write_one_frame(rgb=rgb_np, src_format='rgb')

    if save_decoded_frame:
        # Line 337-348 is modified by ByteDance
        if args['dist_in_yuv420']:
            avg_bpp = sum(bits) / len(bits) / pic_width / pic_height
            avg_psnr_yuv420 = sum(psnrs_yuv420) / len(psnrs_yuv420)
            avg_psnr_rgb = sum(psnrs_rgb) / len(psnrs_rgb)
            folder_name = f"{args['rate_idx']}_{avg_bpp:.4f}_{avg_psnr_yuv420:.4f}_{avg_psnr_rgb:.4f}"
            os.rename(args['recon_path'], args['recon_path'] + f'/../{folder_name}')
        else:
            avg_bpp = sum(bits) / len(bits) / pic_width / pic_height
            avg_psnr_yuv444 = sum(psnrs_yuv444) / len(psnrs_yuv420)
            avg_psnr_rgb = sum(psnrs_rgb) / len(psnrs_rgb)
            folder_name = f"{args['rate_idx']}_{avg_psnr_yuv444:.4f}_{avg_bpp:.4f}_{avg_psnr_rgb:.4f}"
            os.rename(args['recon_path'], args['recon_path'] + f'/../{folder_name}')
    ...
    if args['dist_in_yuv420']:
        log_result = generate_log_json(frame_num, frame_pixel_num, test_time,
                                       frame_types, bits, psnrs_rgb,psnrs_yuv420, msssims_yuv420,
                                       psnrs_y, psnrs_u, psnrs_v,
                                       msssims_y, msssims_u, msssims_v, verbose=args['output_frame_RD'])
    else:
        log_result = generate_log_json(frame_num, frame_pixel_num, test_time,
                                       frame_types, bits, psnrs_rgb, psnrs_yuv444,  msssims_rgb, msssims_yuv444,
                                       psnrs_y, psnrs_u, psnrs_v,
                                       msssims_y, msssims_u, msssims_v, verbose=args['output_frame_RD'])
    return log_result
...
def main():
    ...
    # Line 503-515 is modified by ByteDance
    q_in_ckpt = False
    ...
    for ds_name in config:
        ...
        for seq_name in config[ds_name]['sequences']:
            ...
            for rate_idx in range(rate_num):
                ...
                # Line 533 is modified by ByteDance
                cur_args['output_frame_RD'] = args.output_frame_RD
