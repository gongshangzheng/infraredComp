...
├── datasets/{raw,contour}  # raw=用户视频;contour=阶段1 产物
...
1. **阶段1 提取**:`原始视频(mp4/帧目录)→ [可插拔提取器:canny/sobel] → 无损灰度 PNG 帧序列 + manifest`
   产物存 `datasets/contour/<source>/`,既是阶段2 输入也是质量基准。
...
uv run python -m benchmark.video --input datasets/contour/demo --skip-extract                 # 仅阶段2
